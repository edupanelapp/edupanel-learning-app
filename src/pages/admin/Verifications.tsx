  ...
  const fetchVerifications = async () => {
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          applicant:users!profile_verifications_applicant_id_fkey(email)
        `)
        .eq('status', 'pending')

      // Faculty can only verify students, HOD can verify faculty
      ...
      const { data: verificationsData, error } = await query

      if (error) throw error

      // Fetch profile data for each verification
        ...
        verificationsData.map(async (verification) => {
          ...
          if (verification.role === 'student') {
            const { data } = await supabase
              ...
              .select('full_name, department, student_id')
              .eq('user_id', verification.applicant_id)
              .single()
            profileData = data
          } else if (verification.role === 'faculty') {
            const { data } = await supabase
              .from('faculty_profiles')
              .select('full_name, department, employee_id, designation')
              .eq('user_id', verification.applicant_id)
              .single()
            profileData = data
          }
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          comments: comments[verificationId] || null
        })
